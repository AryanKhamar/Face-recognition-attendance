...
def start():
    ...
    if 'face_recognition_model.pkl' not in os.listdir('static'):
        return render_template('home.html', names=names, rolls=rolls, times=times, l=l,
                               ...
                               mess='There is no trained model. Please add a face first.')

    cap = cv2.VideoCapture(0)

    # Create named OpenCV window and place it
    cv2.namedWindow("Attendance", cv2.WINDOW_NORMAL)
    cv2.moveWindow("Attendance", 100, 100)
    cv2.setWindowProperty("Attendance", cv2.WND_PROP_TOPMOST, 1)

    # ✅ Auto focus the OpenCV window using pyautogui
    time.sleep(1)  # Give the window time to appear
    pyautogui.moveTo(150, 150)  # Coordinates where Attendance window appears
    pyautogui.click()           # Click to focus it
...
def add():
    ...
    cv2.moveWindow("Adding new User", 100, 100)
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if j % 5 == 0:
                if i < nimgs: # Add this condition to stop at nimgs
        ...
        if i >= nimgs or cv2.waitKey(1) == 27:  # Change this condition to check 'i'
            break
